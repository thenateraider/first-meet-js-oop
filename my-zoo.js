...
class Animal {
  ...
  // เพิ่ม helper method สำหรับส่งคืนสถานะของสัตว์
  getStatus() {
    return `Species: ${this.species}, Hunger: ${this.hunger}`;
  }
}
...
class Zoo {
  ...
  addAnimal(animal) {
    ...
    console.log(`Added ${animal.name} the ${animal.species} to ${this.zooName}.`);
  }
...
console.log("--- REGISTERING ANIMALS ---");
myZoo.addAnimal(leo);
myZoo.addAnimal(baloo);
myZoo.addAnimal(zazu);
myZoo.addAnimal(pascal);
myZoo.addAnimal(wolverine);
